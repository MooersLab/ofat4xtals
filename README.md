...
- Autogenerated plots of the results against the factor levels to accelerate the identification of the active factors. The user only enters the lengths of the longest crystal per drop (if any crystals are present) and the crystal score.
...
After doing so, the user does not have to do any more work because the inserted values are plotted in 12 plots shown in the spreadsheet's bottom left-hand corner.
...
This pair of plots shows responses when the crystals are long enough to be measured. 
This comparison shows the value of plotting the crystal scores
because the response positively correlates with the rising factor level.
